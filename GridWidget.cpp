...
GridWidget::GridWidget(int rows, int cols, QWidget *parent /*= nullptr*/)
	...
	,m_flawCountThreshold(10)
	,m_consecutive_doffs(20)
{
...
{
	m_points.clear();
	QMap<int, QList<QPoint>>historyDoffPoint;
	if (m_count_type == 1) {
		for (const auto&point : points)
		{
			QPoint adjustedPoint = adjustPointToGrid(point);//处理一下不合法的点比如负数或超过了最大值
			m_points.append(adjustedPoint);
		}
	}
	else if (m_count_type == 2) {
		static int doff_id = 1;//用于计数，从第1片开始，是否是新一片的片子的缺陷坐标点
		historyDoffPoint[doff_id] = points;
		m_doffInfo.enqueue(historyDoffPoint);
		while (m_doffInfo.size() > m_consecutive_doffs) {
			m_doffInfo.dequeue();
		}
		doff_id++;//下一片
	}
	for (const QMap<int, QList<QPoint>>& doffMap : m_doffInfo) {
		for (auto it = doffMap.begin(); it != doffMap.end(); ++it) {
			int doff_id = it.key();
			const QList<QPoint>& points = it.value();
			qDebug() << "doff_id:" << doff_id << "points:" << points << __FUNCTION__;
		}
	}
	//统计每个网格中点的数量
			...
			for (auto it = doffMap.begin(); it != doffMap.end(); ++it) {
				...
				for (int r = 0; r < m_rows; ++r) {
					for (int c = 0; c < m_cols; ++c) {
						...
						if (m_pointCounts[r][c] > m_flawCountThreshold) {//预留报警，该网格缺陷超过设定阈值
							hasWarn = true;
						}
					}
				}			
