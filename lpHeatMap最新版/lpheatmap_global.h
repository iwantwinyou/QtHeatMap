...
public:
	...
	virtual void setReSetBtn(bool is_show) = 0;
	virtual void handleJobInfo(const QVariantMap& jobInfo) = 0;
public slots:
	virtual void onReset() = 0;
	virtual void onRecvDoffHeatMap(QSharedPointer<QJsonObject>json_sptr) = 0;
	virtual void onUpdateHeatMapData(const QMap<int, QList<QPoint>>& channelDataMap) = 0;
signals:
	void sgAddUiLog(QString info, int type, QVariant vparam = QVariant());
public:
	int m_rows;
	int m_cols;
	int m_gridWidth;
