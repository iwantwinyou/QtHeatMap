...
#include "lpSheetUiNet/include/lpsheetuinet_global.h"
#if _DEBUG 
#pragma comment (lib, "lpSheetUiNet/lib/lpSheetUiNetd.lib")
#else
#pragma comment (lib, "lpSheetUiNet/lib/lpSheetUiNet.lib")
#endif
class GriWidget;
...
public:
	...
	void setReSetBtn(bool is_show)override;
	void handleJobInfo(const QVariantMap& jobInfo)override;//处理获取到的工单信息
public slots :
	//收到一张片子的信息
	void onRecvDoffHeatMap(QSharedPointer<QJsonObject>json_sptr)override;
	//根据收到片子的坐标信息更新热力图数据
	void onUpdateHeatMapData(const QMap<int, QList<QPoint>>& channelDataMap)override;
	//复位，清除点累计个数
	void onReset()override;
	void onGetWarnInfo();//获取报警信息比如缺陷数跟坐标
signals:
